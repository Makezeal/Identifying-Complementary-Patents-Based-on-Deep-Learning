...
def parse_uspto_file(bs, logging=False):
    ...
    for parties in bs.find_all('us-parties'):
        for inventors in parties.find_all('inventors'):
    ...
    # 新添加（由于SQL语句中命名为references有问题，所以统一用的单数形式）
    reference = []
    for refers in bs.find_all('us-references-cited'):
        for refer in refers.find_all('us-citation'):
            for el in refer.find_all('document-id'):
                country = el.find('country').text
                number = el.find('doc-number').text
                reference.append(country + number)
    ...
    uspto_patent = {
        ...
        "claims": claims,  # list
        "text_contention": publication_title + str(abstracts) + str(descriptions) + str(claims)
    }
...
def write_to_db(uspto_patent, db=None):
    ...
    keywords = ['new energy vehicle', 'electric vehicle', 'fuel vehicle',
                'new energy automobile', 'electric automobile', 'fuel automobile']
    def judging(keywords):
        if any(uspto_patent['text_contention'].lower().find(keyword) >= 0 for keyword in keywords):
            return True
    if judging(keywords) == True and uspto_patent['application_type'] == 'utility': # Invention patents for new energy vehicles
        # INSERTS INTO DB
        uspto_db_entry = [
            ...
            ','.join(uspto_patent['reference']), # New addition
        ...
        if db_cursor is not None:             # 这里修改表格名称，新添加一个%s表示reference
            db_cursor.execute("INSERT INTO patent_2022 ("
                              + "publication_title, publication_number, "
                              + "publication_date, publication_type, "
                              + "authors, reference, sections, section_classes, "
                              + "section_class_subclasses, "
                              + "section_class_subclass_groups, "
...
arg_filenames = os.listdir('D:/Code/Data') # Modify the location of the XML file
filenames = []
...
for filename in filenames:
    if ".xml" in filename:
        # Open the XML files one by one
        xml_text = html.unescape(open('D:/Code/Data/'+filename, 'r').read())
