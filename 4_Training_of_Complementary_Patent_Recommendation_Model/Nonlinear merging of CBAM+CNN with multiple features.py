...
# @File : Nonlinear merging of CBAM+CNN with multiple features.py
...
# Configuration logs
logging.basicConfig(filename='/tmp/doc2vec_compgcn.log', level=logging.INFO)
...
class Complementarity_Model(Model):
    ...
    def model(self, input_shape):
        ...
        # 4. Dimensionality reduction, converting matrices into vectors+fully connected layers
        X = Flatten()(X)
...
def model_train_fit(patent_list, labels):
    ...
    for pat in patent_list:
        a, b = pat[0], pat[1]
        text_a, stru_a = text_vectors[a], structure_vectors[a]
        # text_a.extend(stru_a)
        merged_a = [x for x in text_a] + [x for x in stru_a]
        text_b, stru_b = text_vectors[b], structure_vectors[b]
        # text_b.extend(stru_b)
        merged_b = [x for x in text_b] + [x for x in stru_b]
        con_vector = [merged_a, merged_b]
    ...
    # Divide the dataset (if conditions permit later, try the ten fold cross validation method again)
    ...
    complementarity_model.compile(optimizer=Adam(learning_rate=0.001), loss="binary_crossentropy",
                                  metrics=['acc', 'mse', 'mae', km.f1_score(), km.binary_precision(), km.binary_recall()])
    ## 3.Train the model (please note that this operation will take approximately 6-10 minutes)
    ...
    recall = preds[6]
    ## 5.Forecast results
    # prediction = x.reshape((x.shape[0], x.shape[1], x.shape[2], 1))
    # result = complementarity_model.predict(prediction)
    return errors, accuracy, mse, mae, f1_score, precision, recall
...
text_vector = pd.read_csv('/专利向量_doc2vec.csv', index_col=False, names=['patent_num', 'vector'])
structure_vector = pd.read_csv('/tmp/专利向量_compgcn.csv', index_col=False, names=['patent_num', 'vector'])
print('Data read in')
## Vector normalization
text_vector['vectors_normalized'] = text_vector['vector'].apply(normalize_vector)
structure_vector['vectors_normalized'] = structure_vector['vector'].apply(normalize_vector)

## Convert to dictionary format
text_vectors = text_vector.set_index(['patent_num'])['vectors_normalized'].to_dict()
structure_vectors = structure_vector.set_index(['patent_num'])['vectors_normalized'].to_dict()

## Obtain a list of all patents
patent = text_vector['patent_num'].tolist()[:100]
patent_sample = []
label_ = []
for a in patent:
    for b in patent:
...
while j + 1000000 < len(patent_sample):
    i += 1
    log_print('The {} th cycle'.format(i))
