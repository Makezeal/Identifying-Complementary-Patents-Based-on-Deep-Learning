...
class Runner(object):
	...
	def load_data(self):
		...
		# 按照比例划分数据集，并存储到patent_data文件夹下
		p_train, p_test, p_val = [], [], []
		file = open('../data/2022/专利异构知识图.txt', 'r+')
		...
		for split in ['train', 'test', 'valid']:
			file_handle = open('../data/2022/{}.txt'.format(split), 'w')
			if split == 'train':
				for str_Result in p_train:
					file_handle.write(str_Result)
			elif split == 'test':
				for str_Result in p_test:
					file_handle.write(str_Result)
			else:
				for str_Result in p_val:
					file_handle.write(str_Result)
			file_handle.close()


		ent_set, rel_set = OrderedSet(), OrderedSet()
		self.headnode = OrderedSet()
		for split in ['train', 'test', 'valid']:
			for line in open('../data/2022/{}.txt'.format(split)):
				sub, rel, obj = map(str.lower, line.strip().split('\t'))
				ent_set.add(sub)
				rel_set.add(rel)
				ent_set.add(obj)
				self.headnode.add(sub)
	...
	def predict(self, split='valid', mode='tail_batch'):
		...
		with torch.no_grad():
			...
			for step, batch in enumerate(train_iter):
				...
				sub_emb, rel_emb, pred = self.model.forward(sub, rel)
				for i in range(0, len(sub)):
					sub_name = get_dict_key(self.ent2id, int(sub[i]))
					self._embeddings[sub_name] = sub_emb[i]
					rel_name = get_dict_key(self.ent2id, int(rel[i]))
					self._embeddings[rel_name] = rel_emb[i]
	...
	def fit(self):
		...
		save_path = os.path.join(r'../checkpoint/', self.p.name)
	...
	def get_embeddings(self, ):
		...
		for word in self.headnode:
			self.head_embeddings[word] = self._embeddings[word]
...
if __name__ == '__main__':
	...
	fileName = "../data/2022/专利向量_compgcn.csv"
	vec_dict = {}

	for type_ in ['train', 'test', 'valid']:
		res = model.predict(split=type_)
		vec = model.get_embeddings()
		for key, value in vec.items():
			name = key.replace('/m/01 ', '').upper()
			if name not in vec_dict:
				tensor = torch.tensor(value)
				linear_layer = nn.Linear(in_features=len(tensor), out_features=100).to("cuda")  # 统一维度
				result_1 = linear_layer(tensor)
				result_2 = result_1.detach().cpu().numpy()
				vec_dict[name] = result_2
